...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LitePal.getDatabase();//打开数据库
        title= (TextView)findViewById(R.id.title);
        context = getApplicationContext();
        FloatingActionButton fabAdd =findViewById(R.id.fab_add);//悬浮窗
        if (flag ==0x00000000 ){    //如果是今天
            ...
            plan = LitePal.where("Day <= ?",str).find(Plan2.class);//从数据库查找数据
            adapter = new PlanAdapter2(context,plan);
        }
        ...
        if (flag ==0x00000002){ //如果是本月
            ...
            plan = LitePal.where("month=?",str).find(Plan2.class);//在数据库查找
            adapter = new PlanAdapter2(context,plan);
        }
        if (flag ==0x00000003){   //如果是已完成
            title.setText("已完成计划");//标题
            fabAdd.setVisibility(View.GONE);
            plan = LitePal.where("isCompleted = ?","1").find(Plan2.class);
            adapter = new PlanAdapter2(context,plan);
        }
        if (flag ==0x00000004){ //如果是未完成
            title.setText("未完成计划");//标题
            fabAdd.setVisibility(View.GONE);
            plan = LitePal.where("isCompleted = ?","0").find(Plan2.class);
            adapter = new PlanAdapter2(context,plan);
        }
        if (flag ==0x00000005){ //如果是自定义日期
            ...
            plan = LitePal.where("day = ? and month = ? and year = ?",str3,str2,str).find(Plan2.class);
            adapter = new PlanAdapter2(context,plan);
        }
...
//            adapter = new PlanAdapter2(context,plan);
//        }
//        if (flag == 0x00000003){    //如果是已完成计划
//            fabAdd.setVisibility(View.GONE);
//            plan = LitePal.where("isCompleted = ?","1").find(Plan2.class);
//            adapter = new PlanAdapter2(context,plan);
//        }
//        if (flag == 0x00000004){    //如果是未完成计划
//            fabAdd.setVisibility(View.GONE);
//            plan = LitePal.where("isCompleted = ?","0").find(Plan2.class);
//            adapter = new PlanAdapter2(context,plan);
//        }
//        if (flag == 0x00000005){//如果是指定日期
//            int year = bundle.getInt("year");
//            int month = bundle.getInt("month")+1;
//            int day = bundle.getInt("dayOfmonth");//获取年月日信息
//            String str = String.valueOf(year);
...
//        }
    ...
    @Override
    public boolean onOptionsItemSelected(MenuItem item) {//打开侧滑栏
